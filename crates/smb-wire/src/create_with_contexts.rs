    ...
    let contexts_offset =
        SMB2_HEADER_SIZE
            ...
            .ok_or(WireError::InvalidField(
                "CREATE CreateContextsOffset overflow",
            ))?;
    put_u32(
        &mut body,
        48,
...
pub fn decode_create_response_with_contexts(
    ...
    let minimum_offset = SMB2_HEADER_SIZE
        ...
        .ok_or(WireError::InvalidField(
            "CREATE response fixed-size overflow",
        ))?;
...
mod tests {
    ...
    fn request_places_contexts_on_an_eight_byte_boundary_after_the_name() {
        ...
        assert_eq!(
            decode_create_contexts(&message[offset..offset + len]).unwrap(),
            vec![context]
        );
    }
    ...
    fn root_request_uses_context_list_as_the_required_buffer() {
        ...
        let message =
            encode_create_request_with_contexts(&request, &[context], 4, 7, 9, 8).unwrap();
        let body = &message[SMB2_HEADER_SIZE..];
    ...
    fn response_contexts_are_decoded_from_the_declared_slice() {
        ...
        let contexts =
            encode_create_contexts(&[CreateContext::new(b"DH2Q".to_vec(), vec![0x33; 8]).unwrap()])
                .unwrap();
