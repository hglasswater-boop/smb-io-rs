    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidStructureSize { expected, actual } => {
                write!(
                    f,
                    "invalid structure size: expected {expected}, got {actual}"
                )
            }
            ...
            Self::FrameTooLarge(len) => {
                write!(f, "SMB direct-TCP frame exceeds 24-bit length: {len}")
            }
            Self::TruncatedFrame { declared, actual } => {
                write!(
                    f,
                    "truncated SMB frame: declared {declared} bytes, got {actual}"
                )
            }
